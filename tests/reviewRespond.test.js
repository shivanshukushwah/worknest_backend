...
test('PUT /api/reviews/:id/respond accepts responseComment alias', async () => {
  ...
  const review = await Review.create({ job: job._id, reviewer: employer._id, reviewee: student._id, rating: 5, comment: 'Great', isPublic: true })

  const token = jwt.sign({ id: student._id, email: student.email, role: student.role }, process.env.JWT_SECRET)

  // Send with responseComment instead of comment
  const res = await request(app)
    .put(`/api/reviews/${review._id}/respond`)
    .set('Authorization', `Bearer ${token}`)
    .send({ responseComment: 'Thanks for the feedback!' })
    .expect(200)

  expect(res.body.success).toBe(true)
  expect(res.body.data).toHaveProperty('response')
  expect(res.body.data.response.comment).toBe('Thanks for the feedback!')
})