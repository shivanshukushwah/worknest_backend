...
### 0. Registration flow hardened
- Temporary user records are stored only until OTP verification completes.
- Added TTL index (`emailOtpExpires`) so unverified accounts are automatically
  removed when the OTP window passes, preventing ghost users.
- Controller now proactively deletes expired unverified records and provides a
  clearer `409` message when a pending registration exists.
...
### Before (Old System)
```javascript
user.emailOtp = "123456"  // Raw OTP stored
user.emailOtpHash = "hashed..."
user.emailOtpExpires = Date
user.emailOtpAttempts = 3
user.emailOtpBlocked = false
user.emailOtpSentAt = Date
```

### After (Twilio Verify System)
```javascript
user.isEmailVerified = true/false  // Only this needed
user.emailOtpSentAt = Date  // For rate limiting only
```
